...
fn test_target_parser_cpu_efficiency() {
    ...
    assert!(
        avg_micros < 100.0,
        "Average target parse time: {:.2}µs",
        avg_micros
    );
}
...
fn test_timing_template_cpu_efficiency() {
    ...
    assert!(
        avg_micros < 50.0,
        "Average timing template time: {:.2}µs",
        avg_micros
    );
}
...
async fn test_profiler_overhead() {
    ...
    assert!(
        avg_micros < 100.0,
        "Average profiler overhead: {:.2}µs",
        avg_micros
    );
...
fn test_serialization_cpu_efficiency() {
    use chrono::Utc;
    use nemue::reporting::{
        ComplianceStatus, ExecutiveSummary, Finding, ReportBuilder, ReportMetadata, Severity,
    ...
    assert!(
        avg_micros < 500.0,
        "Average JSON serialization time: {:.2}µs",
        avg_micros
    );
}
...
fn test_text_generation_cpu_efficiency() {
    use chrono::Utc;
    use nemue::reporting::{
        ComplianceStatus, ExecutiveSummary, ReportBuilder, ReportMetadata, VulnerabilitySummary,
    };
    ...
    assert!(
        avg_micros < 500.0,
        "Average text generation time: {:.2}µs",
        avg_micros
    );
}
