...
mod tests {
    ...
    fn test_cyclonedx_format() {
        ...
        let json: serde_json::Value =
            serde_json::from_str(&CycloneDxReportGenerator::generate(&report)).unwrap();
    ...
    fn test_cyclonedx_severity_mapping() {
        assert_eq!(
            CycloneDxReportGenerator::severity_to_cyclonedx(&Severity::Critical),
            "critical"
        );
        assert_eq!(
            CycloneDxReportGenerator::severity_to_cyclonedx(&Severity::High),
            "high"
        );
        assert_eq!(
            CycloneDxReportGenerator::severity_to_cyclonedx(&Severity::Medium),
            "medium"
        );
        assert_eq!(
            CycloneDxReportGenerator::severity_to_cyclonedx(&Severity::Low),
            "low"
        );
        assert_eq!(
            CycloneDxReportGenerator::severity_to_cyclonedx(&Severity::Info),
            "info"
        );
    }
