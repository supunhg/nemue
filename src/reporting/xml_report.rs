...
impl XmlReportGenerator {
    ...
    fn write_statistics(xml: &mut String, report: &ScanReport) {
        ...
        let avg_cvss = if total_findings > 0 {
            report
                ...
                / report
                    .findings
                    .iter()
                    .filter(|f| f.cvss_score.is_some())
                    .count()
                    .max(1) as f64
        } else {
            0.0
        };
        let total_affected_hosts: usize =
            report.findings.iter().map(|f| f.affected_hosts.len()).sum();
        ...
        xml.push_str(&format!(
            "    <average_cvss>{:.1}</average_cvss>\n",
            avg_cvss
        ));
        xml.push_str(&format!(
            "    <total_affected_host_instances>{}</total_affected_host_instances>\n",
