...
impl PacketFragmenter {
    ...
    pub fn fragment(&mut self, payload: &[u8]) -> Result<Vec<IpFragment>> {
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            ...
            let fragment = IpFragment::new(chunk.to_vec(), offset, !is_last, fragment_id);
    ...
    pub fn fragment_syn_packet(&mut self, target: IpAddr, port: u16) -> Result<Vec<IpFragment>> {
...
impl FragmentReassembler {
    ...
    pub fn add_fragment(&mut self, fragment: IpFragment) -> Option<Vec<u8>> {
        ...
        self.fragments.entry(id).or_default().push(fragment);
