...
impl WebVulnScripts {
            ...
            if let Ok(mut stream) = TcpStream::connect_timeout(&addr.parse().unwrap(), timeout) {
                ...
                if stream.write_all(request.as_bytes()).is_ok() {
                    ...
                    return Ok(VulnResult {
                        ...
                        cve_ids: vec![],
                        description:
                            "Cross-site scripting allows injection of malicious scripts"
                                .to_string(),
                        evidence: if vulnerable {
                        ...
                        remediation: Some(
                            "Encode all user input and use Content-Security-Policy".to_string(),
                        ),
                        references: vec![
        ...
        .with_executor(|target, port| {
            ...
            if let Ok(mut stream) = TcpStream::connect_timeout(&addr.parse().unwrap(), timeout) {
                ...
                if stream.write_all(request.as_bytes()).is_ok() {
                    ...
                    let vulnerable =
                        response.contains("root:x:0:0") || response.contains("[extensions]");
                    ...
                    return Ok(VulnResult {
                        ...
                        severity: VulnSeverity::High,
                        cve_ids: vec![],
                        description: "Directory traversal allows unauthorized file access"
                            .to_string(),
                        evidence: if vulnerable {
                        ...
                        remediation: Some(
                            "Validate and sanitize file paths, use whitelisting".to_string(),
                        ),
                        references: vec![
    ...
    pub fn command_injection() -> VulnScript {
        ...
        .with_services(vec!["http".to_string(), "https".to_string()])
        .with_executor(|target, port| {
            let _payloads = ["; ls", "| dir", "`whoami`", "$(sleep 5)"];
            ...
            Ok(VulnResult {
                ...
                evidence: None,
                remediation: Some(
                    "Never pass user input to system commands, use safe APIs".to_string(),
                ),
                references: vec![
    ...
    pub fn ssrf_detection() -> VulnScript {
        ...
        .with_executor(|target, port| {
            Ok(VulnResult {
                ...
                evidence: None,
                remediation: Some(
                    "Validate and whitelist URLs, disable unnecessary protocols".to_string(),
                ),
                references: vec![
