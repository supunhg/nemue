...
impl DefaultCredentials {
    ...
    fn populate(&mut self) {
        // Database Systems
        self.add_credentials("mysql", vec![
        ...
        self.add_credentials("tomcat", vec![
        ...
        self.add_credentials("jboss", vec![
            ("admin", "admin", "JBoss default"),
        ]);

        self.add_credentials("weblogic", vec![
            ("system", "password", "WebLogic default"),
            ("weblogic", "weblogic", "WebLogic default"),
        ]);

        // Message Queues
        self.add_credentials("rabbitmq", vec![
            ("guest", "guest", "RabbitMQ default"),
            ("admin", "admin", "Common default"),
        ]);

        self.add_credentials("activemq", vec![
            ("admin", "admin", "ActiveMQ default"),
        ]);

        // Network Devices
        self.add_credentials("cisco", vec![
            ("admin", "admin", "Cisco default"),
            ("cisco", "cisco", "Cisco default"),
            ("admin", "", "No password"),
        ]);

        self.add_credentials("juniper", vec![
            ("netscreen", "netscreen", "Juniper default"),
            ("admin", "admin", "Juniper default"),
        ]);

        // SSH/Telnet
        self.add_credentials("ssh", vec![
            ("root", "root", "Common default"),
            ("admin", "admin", "Administrative account"),
        ...
        self.add_credentials("telnet", vec![
            ("admin", "admin", "Administrative account"),
            ("root", "root", "Root account"),
        ]);

        // FTP
        self.add_credentials("ftp", vec![
        ...
        self.add_credentials("nagios", vec![
            ("nagiosadmin", "nagiosadmin", "Nagios default"),
            ("nagios", "nagios", "Nagios default"),
        ]);

        self.add_credentials("zabbix", vec![
            ("Admin", "zabbix", "Zabbix default (case sensitive)"),
            ("admin", "admin", "Common default"),
        ]);

        self.add_credentials("grafana", vec![
            ("admin", "admin", "Grafana default"),
        ]);

        self.add_credentials("jenkins", vec![
            ("admin", "admin", "Jenkins default"),
            ("admin", "password", "Common default"),
        ]);

        // Containers & Orchestration
        self.add_credentials("docker", vec![
            ("admin", "admin", "Docker registry"),
        ]);

        self.add_credentials("kubernetes", vec![
            ("admin", "admin", "K8s dashboard"),
        ]);

        // VPN & Remote Access
        self.add_credentials("openvpn", vec![
            ("admin", "admin", "OpenVPN default"),
        ]);

        self.add_credentials("vpn", vec![
            ("admin", "admin", "Generic VPN"),
            ("vpn", "vpn", "VPN default"),
        ]);

        // IoT & Cameras
        self.add_credentials("camera", vec![
        ...
        self.add_credentials("iot", vec![
            ("admin", "admin", "IoT device default"),
            ("root", "root", "Root account"),
        ]);

        // Web Applications
        self.add_credentials("wordpress", vec![
            ("admin", "admin", "WordPress default"),
            ("admin", "password", "Common default"),
        ]);

        self.add_credentials("joomla", vec![
            ("admin", "admin", "Joomla default"),
        ]);

        self.add_credentials("drupal", vec![
            ("admin", "admin", "Drupal default"),
        ]);

        // ERP/CRM
        self.add_credentials("sap", vec![
            ("SAP*", "06071992", "SAP default"),
            ("DDIC", "19920706", "SAP default"),
        ]);

        self.add_credentials("salesforce", vec![
            ("admin", "admin", "Salesforce default"),
        ]);

        // Elasticsearch & Search
        self.add_credentials("elasticsearch", vec![
            ("elastic", "changeme", "Elasticsearch default"),
        ]);

        self.add_credentials("kibana", vec![
            ("elastic", "changeme", "Kibana default"),
        ]);

        // Printers
        self.add_credentials("printer", vec![
        ...
        // IPMI/BMC
        self.add_credentials("ipmi", vec![
            ("ADMIN", "ADMIN", "IPMI default"),
            ("admin", "admin", "Common default"),
        ]);

        // General/Other
        self.add_credentials("default", vec![
...
mod tests {
    ...
    fn test_default_credentials() {
        ...
        assert!(db.count() > 50, "Should have 50+ credentials, got {}", db.count());
        assert!(db.service_count() > 20, "Should have 20+ services, got {}", db.service_count());
    }
