...
impl SecurityHeaderAnalyzer {
    ...
    fn check_x_frame_options(headers: &HashMap<String, String>) -> SecurityHeaderResult {
        ...
        if let Some(value) = Self::get_header_case_insensitive(headers, header_name) {
            ...
            SecurityHeaderResult {
                ...
                present: true,
                value: Some(value.clone()),
                severity: if is_valid { Severity::Info } else { Severity::Medium },
    ...
    fn check_x_content_type_options(headers: &HashMap<String, String>) -> SecurityHeaderResult {
        ...
        if let Some(value) = Self::get_header_case_insensitive(headers, header_name) {
            ...
            SecurityHeaderResult {
                ...
                present: true,
                value: Some(value.clone()),
                severity: if is_nosniff { Severity::Info } else { Severity::Low },
    ...
    fn check_x_xss_protection(headers: &HashMap<String, String>) -> SecurityHeaderResult {
        ...
        if let Some(value) = Self::get_header_case_insensitive(headers, header_name) {
            SecurityHeaderResult {
                ...
                recommendation: "Header present (Note: CSP is preferred over X-XSS-Protection)".to_string(),
            }
        } else {
            SecurityHeaderResult {
                header: "X-XSS-Protection".to_string(),
                ...
                recommendation: "Add 'X-XSS-Protection: 1; mode=block' (though CSP is preferred)".to_string(),
            }
        }
    }
    ...
    fn check_referrer_policy(headers: &HashMap<String, String>) -> SecurityHeaderResult {
        ...
        if let Some(value) = Self::get_header_case_insensitive(headers, header_name) {
            ...
            let is_secure = secure_policies.iter().any(|p| value.to_lowercase().contains(p));

            SecurityHeaderResult {
                header: "Referrer-Policy".to_string(),
                present: true,
                value: Some(value.clone()),
                severity: if is_secure { Severity::Info } else { Severity::Low },
                recommendation: if is_secure {
                    "Header properly configured".to_string()
                } else {
                    "Use a more restrictive policy like 'strict-origin-when-cross-origin'".to_string()
                },
            }
        } else {
            SecurityHeaderResult {
    ...
    fn check_information_disclosure(headers: &HashMap<String, String>) -> Vec<SecurityHeaderResult> {
        let mut results = Vec::new();
        ...
        if let Some(powered_by) = Self::get_header_case_insensitive(headers, "x-powered-by") {
            results.push(SecurityHeaderResult {
                ...
                severity: Severity::Low,
                recommendation: "Remove X-Powered-By header to prevent information disclosure".to_string(),
            });
        }
...
mod tests {
    ...
    fn test_weak_hsts() {
        let mut headers = HashMap::new();
        headers.insert("Strict-Transport-Security".to_string(), "max-age=300".to_string());
